...
var splash = false
var hit = []
var my_fleet_destroy = []
var enemy_fleet_destroy = []
var player1_pos
var player2_pos
...
var next_text_index
var enemy_action
var enemy_action_fleet_id
var enemy_attack_position
var enemy_move_direction
var enemy_move_distance
...
actionListner9 = function () {
    ...
    if(s==0) {
        ...
        my_fleet_destroy = update_attack_status(text_, my_fleet_destroy)
        print_attack_status(my_fleet_destroy)
    }
    else if(s==1){
        clear_fleet_on_map()
...
text_temp = "配置が完了していません"           // 17
game_texts.push(make_text(text_temp))

text_temp = "同じ場所には配置できません"       // 18
game_texts.push(make_text(text_temp))
...
print_attack_status = function(destroy){
    var text
    var fleet_type
    print_status_message_i = 0
    status_messages = []
    if(turn==-1){
        text = "相手の"+fleet_type_for_id(enemy_action_fleet_id)+"の攻撃！"
        status_messages.push(make_status_message(text))
        console.log(text)
    }else{
        text = fleet_type_for_id(selected_fleet().id)+"の攻撃！"
        status_messages.push(make_status_message(text))
        console.log(text)
    }
    ...
    if(hit.length>0){
        for(var i=0; i<hit.length; i++){
            ...
            //
            if(turn==1) text = fleet_type+"にヒット!"
            else text = fleet_type+"に被弾！"
            status_messages.push(make_status_message(text))
            console.log(text)
            if(destroy.indexOf(hit[i])!=-1){
                fleet_type = fleet_type_for_id(hit[i])
                //
                if(turn==1) text = fleet_type+"を撃沈させた！"
                else text = fleet_type+"が撃沈！"
                status_messages.push(make_status_message(text))
                if(destroy.length>=3){
                    //
                    if(turn==1) console.log("勝利")
                    else console.log("敗北")
                    game_end = true
                }
            }
...
print_enemy_action_status = function(){
    var text
    var fleet_name = fleet_type_for_id(enemy_action_fleet_id)
    print_status_message_i = 0
    status_messages = []

    if(enemy_action=="a"){
        attack_status = get("AttackResponse2")
        attack_status = get("AttackResponse2")
        attack_status = get("AttackResponse2")
        enemy_fleet_destroy = update_attack_status(attack_status, enemy_fleet_destroy)
        print_attack_status(enemy_fleet_destroy)
    }
    else{
        text = "相手の"+fleet_name+"が"+enemy_move_direction+"に"+enemy_move_distance+"移動しました"
        status_messages.push(make_status_message(text))
        console.log(text)
    }
    count = 0
...
cut_in_enemy_turn = function(){
    if(enemy_turn_text.x<=-1000){
        ...
        var actionCode = get("ActionCpu")
        action_code_parse(actionCode)
...
var print_status_message = function(){
    action_flag *= -1
    if(print_status_message_i>=status_messages.length && count<=360 && turn==-1 && enemy_action=="a"){
        count += 1
    } else if(print_status_message_i>=status_messages.length && count<=130){
        count += 1
        ...
        else { cut_in_enemy_turn() }
        return
    }else{
        stage.addChild(status_messages[print_status_message_i])
        renderer.render(stage)
    }
    if (count==0&&status_messages[print_status_message_i].y >= (box_height+18)*print_status_message_i+50+150){
        status_messages[print_status_message_i].y -= 30
        if(action_flag==1 && turn==-1 && enemy_action=="a")status_messages[print_status_message_i].y += 30
    }
    else if(count==0&&status_messages[print_status_message_i].y >= (box_height+18)*print_status_message_i+50){
        status_messages[print_status_message_i].y -= 20
        if(action_flag==1 && turn==-1 && enemy_action=="a")status_messages[print_status_message_i].y += 20
    }
    else if(count==0){
...
action_code_parse = function(raw_data){
    var parsed_data = raw_data.split('-')
    enemy_action = parsed_data[0]
    enemy_action_fleet_id = parseInt(parsed_data[1])-1
    if(enemy_action=="m"){
        enemy_move_direction = parsed_data[2]
        enemy_move_distance = parsed_data[3]
    }
    else if(enemy_action=="a"){
        enemy_attack_position = [parsed_data[2], parsed_data[3]]
    }
}

update_attack_status = function (raw_data, destroy) {
    count=0
    hit = []
    splash = false
    var parsed_data = []
    ...
    medium_data.forEach(function (value) {
        if(value=="m")splash = true
            ...
            hit.push(0)
            if(turn==-1)fleets_variable[0].hp.text -= 1
        }
        else if(value=="2"){
            hit.push(1)
            if(turn==-1)fleets_variable[1].hp.text -= 1
        }
        else if(value=="3"){
            hit.push(2)
            if(turn==-1)fleets_variable[2].hp.text -= 1
        }
        else if(value=="1d"){
            hit.push(0)
            destroy.push(0)
