...
module Create.Scene{
    export function Title(sceneManager): void {
        // タイトルステージ
    ...
    class Components {
        ...
        public static titleStage: any;
        public static loadBar: any;
        public static startButton: any;
        public static blackScreen: any;
    }
    class Iterator{
        public static i: any;
        public static j: any;
    ...
    function changeWidthLoadBar(){
        ...
        if(Iterator.i>40&&Iterator.j<30)Iterator.j += 1;
        else if(Iterator.i>60)Iterator.i += 2;
        else if(Iterator.i>86)Iterator.i += 1.3;
        else Iterator.i += 1;
    }

    function titleParts() {
        if(Components.startButton.children[0].alpha>=1)return;
    ...
    function blackScreen() {
        if(Iterator.i==320)createjs.Sound.play("title", {loop: -1, volume: GameParameters.Status.soundVolume["music"]*5});
        if(Iterator.i >= 360){
