...
(function (Create) {
    ...
    (function (Component) {
        ...
        Component.gameStartButton = gameStartButton;
        function button() {
            var button;
            button = new PIXI.Graphics();
            button.beginFill(0x000000);
            button.drawPolygon(0, 40, 32, 0, 280, 0, 312, 40, 280, 80, 32, 80);
            button.endFill();
            button.alpha = 0.6;
        ...
        function actionCmdButton(text, type) {
            var buttonStage = new PIXI.Container();
            var button;
            button = new PIXI.Graphics();
            if (type == 0)
                button.beginFill(0xFE4F4F);
            else if (type == 1)
                button.beginFill(0x03EF75);
            else
                button.beginFill(0x000000);
            button.drawPolygon([
                80, 0,
        ...
        Component.actionCmdButton = actionCmdButton;
        function text(text) {
        ...
        Component.map = map;
        function fleetIcons(scenesManager) {
            var fleetIcons = new Array();
            var texture;
            for (var i = 0; i < 9; i++) {
                ...
                scenesManager.loader.load(function () {
                    if (i < 3)
                        ...
                        texture = PIXI.Texture.from('/images/char/' + GameParameters.Status.team[i % 3] + '_c.png');
                    else
                        texture = PIXI.Texture.from('/images/char/' + GameParameters.Status.team[i % 3] + '_d.png');
                    fleetIcons[i].addChild(new PIXI.Sprite(texture));
                    if (i >= 6)
                        fleetIcons[i].addChild(gekitinIcon(scenesManager));
                });
                fleetIcons[i].id = i % 3;
                fleetIcons[i].hp = 3 - i;
                ...
                if (i < 3)
                    fleetIcons[i].alpha = GameParameters.Component.DEFAULT_ALPHA;
                if (i < 6) {
        ...
        Component.fleetIcons = fleetIcons;
        function enemyIcons(scenesManager) {
            var enemyIcons = new Array();
            var texture;
            for (var i = 0; i < 3; i++) {
        ...
        Component.enemyIcons = enemyIcons;
        function gekitinIcon(scenesManager) {
            var texture2;
            scenesManager.loader.load(function () {
        ...
        Component.gekitinIcon = gekitinIcon;
        function faceIcons(scenesManager) {
            var faceIcons = new Array();
            var texture;
            for (var i = 0; i < 3; i++) {
        ...
        Component.faceIcons = faceIcons;
        function fleetImages(scenesManager) {
        ...
        Component.fleetImages = fleetImages;
        function explosionAnimation(scenesManager) {
            var sprites = new Array();
            for (var i = 0; i < 17; i++) {
        ...
        Component.explosionAnimation = explosionAnimation;
        function battleStart(scenesManager) {
            var texture;
            texture = scenesManager.loader.load(function () {
        ...
        Component.battleStart = battleStart;
        ...
        function informationHeader(text) {
            var style = new PIXI.TextStyle({
                fontSize: 32,
                fontWeight: "bold",
                fill: 0xffffff,
        ...
        Component.informationHeader = informationHeader;
        ...
        Component.informationText = informationText;
        function informationPanel(scenesManager) {
        ...
        Component.informationPanel = informationPanel;
        function closeIcon(scenesManager) {
            var roundBox = new PIXI.Graphics();
        ...
        function scoreText(logs) {
            var text = "";
            for (var i = 0; i < logs.length; i++) {
                text += "vs CPU: " + logs[i]["turn"] + "ターン ";
                if (logs[i]["winner"] == 0)
                    text += "勝ち";
                else if (logs[i]["winner"] == 1)
                    text += "負け ";
        ...
        Component.scoreText = scoreText;
        function stagePanel(scenesManager) {
        ...
        Component.stagePanel = stagePanel;
        function stagePanel2(scenesManager) {
            var stageStage = new PIXI.Container();
            var stage = new PIXI.Graphics();
        ...
        Component.stagePanel2 = stagePanel2;
        function sallySorties() {
            var sprites = new Array();
            for (var i = 0; i < 45; i++) {
        ...
        Component.sallySorties = sallySorties;
        function backButton() {
        ...
        Component.sallyCommon = sallyCommon;
        ...
        Component.decisionButton = decisionButton;
        function shutugekiButton() {
        ...
        Component.shutugekiButton = shutugekiButton;
        function sallyMapParts() {
            var sprites = new Array();
            for (var i = 0; i < 19; i++) {
        ...
        Component.sallyMapParts = sallyMapParts;
        function textField() {
            var stage = new PIXI.Container();
        ...
        Component.loadBar = loadBar;
        function fleetGigImage(scenesManager) {
            var fleetImages = new Array();
            var texture;
            for (var i = 0; i < 3; i++) {
                fleetImages[i] = new PIXI.Container();
        ...
        Component.fleetGigImage = fleetGigImage;
        function mapCommon() {
            var sprites = new Array();
            for (var i = 0; i < 188; i++) {
        ...
        Component.mapCommon = mapCommon;
        function standbyMessage() {
            var stage = new PIXI.Container();
        ...
        Component.standbyMessage = standbyMessage;
        function standbyMessage2(scenesManager) {
            var sprite;
            scenesManager.loader.load(function () {
        ...
        Component.standbyMessage2 = standbyMessage2;
        function battleCutin(scenesManager) {
            var sprite;
            scenesManager.loader.load(function () {
        ...
        Component.battleCutin = battleCutin;
    })(Component = Create.Component || (Create.Component = {}));
})(Create || (Create = {}));
